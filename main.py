    ...
    if message.author != bot.user and "-" in message.content:
        ...
        if len(quotes) > 0:
            for quote in quotes:
                ...
                viewID_to_submitted_by[viewid] = message.author.nick if message.author.nick is not None else message.author.name
...
class ApproveView(discord.ui.View):
    ...
    async def approve(self, button: discord.ui.Button, interaction: discord.Interaction):
        if self.viewid in have_approved:
            await interaction.response.send_message("Race condition detected - someone else has approved it!", ephemeral=True)
            return
        ...
        embed = discord.Embed(title=f"Quote from {viewID_to_submitted_by[self.viewid]} about {viewID_to_author[self.viewid]}", description='"' + content + '" - ' + author)
